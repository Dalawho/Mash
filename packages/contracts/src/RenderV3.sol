...
contract RenderV3 is Ownable, SSt {
    ...
    function getTraitData(address _collection, uint8 _layerId, uint8 _traitId, bool wholeToken) public view returns(bytes memory) {
        ...
        if(wholeToken) {
            ...
            //on-chain Kevin
            if(_collection == onChainKevin) return bytes(IKevin(onChainKevin).hashToSVG(IKevin(onChainKevin)._tokenIdToHash(id)));//also returns encoding
            //nouns
    ...
    function getNounsToken(uint256 _id) public view returns(bytes memory) {
        //get seed for token
        ...
        return bytes(string.concat("data:image/svg+xml;base64,",out));
    }
    ...
    function _renderImg(LayerStruct memory _currentLayer, CollectionInfo memory _currentCollection, IIndelible.Trait memory traitNames, bytes memory buffer) private view {
        bytes memory _traitData = getTraitData(_currentCollection.collection, _currentLayer.layerId, _currentLayer.traitId, _currentCollection.maxSupply == 99);
        buffer.appendSafe(bytes(string.concat('<image x="', int8ToString(_currentLayer.xOffset), '" y="', int8ToString(_currentLayer.yOffset),'" width="', Strings.toString(_currentCollection.xSize*_currentLayer.scale), '" height="', Strings.toString(_currentCollection.ySize*_currentLayer.scale),
         '" href="')));
        if (_currentCollection.maxSupply != 99) {buffer.appendSafe(bytes(string.concat( 'data:', traitNames.mimetype , ';base64,')));}
        buffer.appendSafe((_currentCollection.collection == onChainKevin || renderers[_currentCollection.collection].base64Encoded || _currentCollection.maxSupply == 99) ? _traitData : bytes(Base64.encode(_traitData)));

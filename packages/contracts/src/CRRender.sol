...
contract ChainRender is Shared {
    ...
    //     //get trait
    //     Layer[NUM_LAYERS] memory _traits;
        
    //     //_traits[0] = cr.getLayer(0,16);
    //     _traits[0] = cr.getLayer(_layerId, _traitId);
    //     // for (uint i = 0; i < 20; i++) {
    //     //     _traits[0].hexString[i] = 0x00;
    //     // }
    //     //get Palette
    //     ICR.Color [NUM_COLORS][NUM_LAYERS] memory _palettes;
    //     // _palettes[0] = palette(_traits[0].hexString);
    //     // _palettes[1] = palette(_traits[1].hexString);
    //     _palettes[0] = palette(_traits[0].hexString);
    ...
    //         tokenSVGBuffer(_traits, _palettes, 1),
    //         "PHN0eWxlPnJlY3R7d2lkdGg6MTBweDtoZWlnaHQ6MTBweDt9PC9zdHlsZT48L3N2Zz4="); //add a = at the end?
    // }
    ...
    // function tokenSVGBuffer(Layer memory tokenLayers, Color [NUM_COLORS] memory tokenPalettes, uint8 numTokenLayers) public pure returns (bytes memory) {
    ...
    //         for(uint i; i < 4; ++i) {
    //             for(uint j; j < 8; ++j) {
    //                 buffer.appendSafe(bytes(pixel(lookup, colorForIndex(tokenLayers, k, 0, tokenPalettes, numTokenLayers), x, y)));
    //                 ++x;
    //             }
    //             k += 3;
    //         }
